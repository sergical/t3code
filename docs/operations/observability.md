...
### Sentry (over OTLP, no SDK)

Set `SENTRY_DSN` on the server to send every Effect span — including the gen_ai
agent spans described below — to that Sentry project over Sentry's OTLP ingest
endpoint. The URL and auth header are derived from the DSN; no Sentry SDK is
involved, so this stays plain OpenTelemetry. It takes precedence over
...
Unset the variable and nothing changes.

Because ingest is spans-only, errors do not become Sentry Issues on this path:
a failed turn is a span with error status inside its trace, not an alertable
issue. A client that runs its own tracing SDK can send a `sentry-trace` header
with each RPC request and the server parents the matching `ws.rpc.*` span on
it; clients without one stay their own trace roots.
...
parented to it and ends when the task reports. The split keeps a
long-lived background task from holding its launch turn's trace open. The subagent's report becomes the input of the follow-up turn that
